...
static void felica_uart_init(void)
{
	...
	FELICA_PR_ERR(" %s START", __func__);
	...
	FELICA_PR_ERR(" %s END, major=[%d], minor=[%d]", __func__,
...
static int felica_uart_open(struct inode *inode, struct file *file)
{
	...
	if (gfa_open_cnt == 0) {
		...
		FELICA_PR_ERR(" %s Wake Lock(2)", __func__);
#endif
	}
	gfa_open_cnt++;
...
static int felica_uart_close(struct inode *inode, struct file *file)
{
	...
	if (gfa_open_cnt == 0) {
		...
		FELICA_PR_ERR(" %s Wake UnLock(2)", __func__);
#endif
	}
				...
				size_t len, loff_t *ppos)
{
	...
	if (FELICA_NL_MSG_DATA_SIZE < wk_len) {
		FELICA_PR_ERR(" %s  read max size over [%d]", __func__,
				 wk_len);
	...
	if (nlret == 0) {
		...
		if (gfa_rcv_str[1] == FELICA_NL_SUCCESS) {
			wk_len =
				...
				(int)gfa_rcv_str[3];
			FELICA_PR_ERR(" %s Rcv len [%d]", __func__, wk_len);
			FELICA_PR_ERR(
			      ...
			      unsigned long arg)
{
	...
	int ret;
	FELICA_PR_ERR(" %s START", __func__);
	FELICA_PR_ERR(" %s cmd = [0x%x]", __func__, cmd);

	/* Ignore [TCGETS:0x5401] and [TCSBRK:0x5409] */
	if (TCGETS == cmd || TCSBRK == cmd) {
		FELICA_PR_ERR(" %s cmd throw [0x%x]", __func__, cmd);
		return 0;
	}
	if (down_interruptible(&dev_sem->felica_sem)) {
		FELICA_PR_ERR(" %s ERROR(down_interruptible)", \
	...
	if (ret == 0) {
		...
		if (gfa_rcv_str[1] == FELICA_NL_SUCCESS) {
			...
			FELICA_PR_ERR(" Available Success data size [%d]", \
				ret_str);
		} else {
...
static int felica_nl_send_msg(int len)
{
	...
	int msg_size = 0;
	FELICA_PR_ERR(" %s START len: %d", __func__,len);
...
static void felica_nl_recv_msg(struct sk_buff *skb)
{
	...
	if (wskb && (wskb->len > NLMSG_SPACE(0))) {
		...
		FELICA_PR_ERR(" %s START gfa_rcv_str[0]: 0x%x", __func__,gfa_rcv_str[0]);
	...
	if (felica_get_tamper_fuse_cmd() != 1)
			...
			FELICA_PR_ERR(" RCV MSG [%d]", gfa_rcv_str[0]);
#endif
			...
			gfa_wait_flag = 1;
				...
				&& (gfa_pid == nlh->nlmsg_pid)) {
			FELICA_PR_ERR("RCV MSG FELICA_INIT_UICC_END [%d]",
			gfa_rcv_str[0]);
			if (gfa_rcv_str[1] == FELICA_NL_SUCCESS)
				FELICA_PR_ERR("UICC INIT END SUCCESS");
			else
				FELICA_PR_ERR("UICC INIT END FAIL");
...
static void felica_nl_wait_ret_msg(void)
{
	...
	while (1) {
		if (gfa_wait_flag == 1) {
			FELICA_PR_ERR(" %s sleep cnt [%d]", __func__,
					 cnt);
			break;
		}
		mdelay(1);
...
static int felica_Cpu0(void)
{
	...
	FELICA_PR_ERR(" System has %d CPU's, we are on CPU #%d\n"
	...
	if (0 != ret)
		FELICA_PR_ERR(" set_cpus_allowed_ptr=%d.\n", ret);

	FELICA_PR_ERR(" And now we are on CPU #%d", raw_smp_processor_id());
...
static uint8_t felica_get_tamper_fuse_cmd(void)
{
	...
	FELICA_PR_ERR(" %s END resp_buf = %d\n",__func__, resp_buf);
...
#endif // CONFIG_MACH_KLTE
#endif
	FELICA_PR_ERR(" %s GPIO Read value of GPIO_PINID_FELICA_PON -> ret=[%d]", __func__,ret);

	if (ret == GPIO_VALUE_HIGH) {
		retparam = FELICA_PON_WIRED;
		FELICA_PR_ERR(" Wired interface Status is [%d][start]",
				 retparam);
	} else if (ret == GPIO_VALUE_LOW) {
		retparam = FELICA_PON_WIRELESS;
		FELICA_PR_ERR(" Wired interface Status is [%d][standby]",
				 retparam);
	} else {
				...
				size_t len, loff_t *ppos)
{
	...
	if (pon == FELICA_PON_WIRED) {
		...
		FELICA_PR_ERR(" Set wired interface to [%d][start]",
				 setparam);
	} else if (pon == FELICA_PON_WIRELESS) {
		setparam = GPIO_VALUE_LOW;
		FELICA_PR_ERR(" Set wired interface to [%d][standby]",
				 setparam);
	} else {
...
static int felica_cen_open(struct inode *inode, struct file *file)
{
	...
	FELICA_PR_ERR(" %s START . system_rev=[%d]", __func__,system_rev);

	#if defined(CONFIG_MACH_HLTEDCM)
	//high
	if (system_rev == HW_REV09_OR_10)
	{
		FELICA_PR_ERR(" %s Making MHL_EN HIGH", __func__);
	...
	FELICA_PR_ERR(" %s END -EACCES, uid=[%d], gmfc_uid=[%d], gdiag_uid=[%d], gmfl_uid=[%d]\n", __func__, uid,gmfc_uid,gdiag_uid,gmfl_uid);
			...
			if (system_rev == HW_REV09_OR_10)
			{
				FELICA_PR_ERR(" %s Making MHL_EN LOW", __func__);
...
static int felica_cen_close(struct inode *inode, struct file *file)
{
	...
	FELICA_PR_ERR(" %s END system_rev=[%d]", __func__,system_rev);
	return 0;
}
			...
			size_t len, loff_t *ppos)
{
	...
	FELICA_PR_ERR(" %s g_cen_sts=[%d]", __func__, g_cen_sts);
				...
				size_t len, loff_t *ppos)
{
	...
	if (cen == FELICA_CEN_UNLOCK) {
		...
		FELICA_PR_ERR(" Set FeliCa-Lock status to [%d][UnLock]",
				 write_buff[1]);
	} else if (cen == FELICA_CEN_LOCK) {
		write_buff[1] = FELICA_CEN_SET_LOCK;
		FELICA_PR_ERR(" Set FeliCa-Lock status to [%d][Lock]",
				 write_buff[1]);
	} else {
		...
		if (0 != g_cen_sts) {
			if (GPIO_VALUE_LOW != g_rfs_sts) {
				if (0 == guartcc_felica_status) {
					available_d->rsp_done = 1;
					FELICA_PR_ERR(" wake up available");
					wake_up_interruptible(
					&available_d->read_wait);
				}
			}
    ...
    up(&cen_sem->felica_sem);
    FELICA_PR_ERR(" %s END, g_cen_sts =%d, g_rfs_sts = %d", __func__, g_cen_sts, g_rfs_sts);
	return FELICA_CEN_DATA_LEN;
}
...
static int felica_rfs_open(struct inode *inode, struct file *file)
{
	...
	FELICA_PR_ERR(" %s END, uid=[%d], gmfc_uid=[%d], gdiag_uid=[%d]",  __func__, uid, gmfc_uid, gdiag_uid);
	return 0;
}
				...
				size_t len, loff_t *ppos)
{
	...
	if (ret == GPIO_VALUE_HIGH) {
		...
		FELICA_PR_ERR(" Carrier-Wave Status is [%d][standby]",
				 retparam);
	} else if (ret == GPIO_VALUE_LOW) {
		retparam = FELICA_RFS_DETECTED;
		FELICA_PR_ERR(" Carrier-Wave Status is [%d][detected]",
				 retparam);
	} else {
		FELICA_PR_ERR(" %s ERROR(gpio_get_value), ret=[%d]",
			       __func__, ret);
			       ...
			       loff_t *ppos)
{
	...
	if (gfelica_rw_status == FELICA_RW_STATUS_ENABLE) {
		...
		FELICA_PR_ERR(" ReaderWriterFunction status is [%d][enabled] %s",
				 retparam,__func__);
...
static void felica_int_irq_work(struct work_struct *work)
{
	...
	FELICA_PR_ERR(" %s Wake Lock(1)[%d]", __func__, tmout_1);
#endif
...
static void felica_int_poll_init(void)
{
	...
	FELICA_PR_ERR(" %s gfelica_irq_int_pin [%d]",
	...
	FELICA_PR_ERR(" %s gfelica_sps_pin [%d]",
	...
	if (system_rev >= g_uicc_initrev) {
		gpio_tlmm_config(GPIO_CFG(gfelica_sps_pin, GPIOMUX_FUNC_GPIO,
				 ...
				 GPIO_CFG_ENABLE);
		FELICA_PR_ERR(" %s gfelica_sps_pin dcm [%d]",
			__func__, gfelica_sps_pin);
		FELICA_PR_ERR(" %s gfelica_sps_pin dcm [%d]",
	...
	FELICA_PR_ERR(" %s INFO(request_irq)= %d GPIO=%d, ret=[%d]",
				    ...
				    size_t len, loff_t *ppos)
{
	...
	if (ret == GPIO_VALUE_HIGH) {
		...
		FELICA_PR_ERR(" INT-PIN value is [%d][HIGH]", retparam);
	} else if (ret == GPIO_VALUE_LOW) {
		retparam = FELICA_INT_LOW;
		FELICA_PR_ERR(" INT-PIN value is [%d][LOW]", retparam);
	} else {
		FELICA_PR_ERR(" %s ERROR(gpio_get_value), ret=[%d]",
			       __func__, ret);
		return -EIO;
...
static int felica_uid_open(struct inode *inode, struct file *file)
{
	...
	if (strncmp(cmdline, gdiag_name, leng) != 0) {
		FELICA_PR_ERR(" %s ERROR, %s gdiag %s", \
...
static long felica_uid_ioctl(struct file *file, unsigned int cmd,
			     unsigned long arg)
{
	FELICA_PR_ERR(" %s START, cmd=[%d]", __func__, cmd);

	switch (cmd) {
	case SET_FELICA_UID_DIAG:
		gdiag_uid = *((int *)arg);
		FELICA_PR_ERR("Set gdiag_uid to [%d]", gdiag_uid);
		break;
	default:
			...
			size_t len, loff_t *ppos)
{
	...
	FELICA_PR_ERR(" %s read_buff=[%d]", __func__, read_buff);
...
static void felica_register_device(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(gfelica_pon_pin, GPIOMUX_FUNC_GPIO,
					GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
	FELICA_PR_ERR(" %s gfelica_pon_pin [%d]",__func__, gpio_get_value(gfelica_pon_pin));
	}
	...
	FELICA_PR_ERR(" %s gfelica_hsel_pin [%d]",__func__, gfelica_hsel_pin);
	...
	if (system_rev >= g_uicc_initrev) 
	{
		gpio_tlmm_config(GPIO_CFG(gfelica_hsel_pin, GPIOMUX_FUNC_GPIO,
				 GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
				 GPIO_CFG_ENABLE);
		FELICA_PR_ERR(" %s gfelica_hsel_pin [%d]",__func__, gpio_get_value(gfelica_hsel_pin));
	}
	...
	FELICA_PR_ERR(" %s gfelica_sps_pin [%d]",__func__, gfelica_sps_pin);
	...
	if (system_rev >= g_uicc_initrev) 
	{
		gpio_tlmm_config(GPIO_CFG(gfelica_sps_pin, GPIOMUX_FUNC_GPIO,
				 GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
				 GPIO_CFG_ENABLE);
		FELICA_PR_ERR(" %s gfelica_sps_pin [%d]",__func__, gpio_get_value(gfelica_sps_pin));
	}
...
#endif
	FELICA_PR_ERR(" %s , system_rev=[%d]", __func__, system_rev);
				...
				size_t len, loff_t *ppos)
{
	...
	if (0 == hsel_val) {
		FELICA_PR_ERR(" %s HSEL = Low", __func__);
	} else if (1 == hsel_val) {
		FELICA_PR_ERR(" %s HSEL = High", __func__);
	} else {
...
static irqreturn_t intu_poll_irq_handler(int irq, void *dev_id)
{
	...
	FELICA_PR_ERR(" %s START irq = %d", __func__, irq);
...
static void intu_poll_work_func(struct work_struct *work)
{
	...
	FELICA_PR_ERR(" %s read_value = %d old_value = %d",
			...
			size_t len, loff_t *ppos)
{
	...
	if (!intu_d->irq_handler_done) {
		...
		FELICA_PR_ERR(" NFC intu_poll wait irq");
				...
				size_t len, loff_t *ppos)
{
	...
	FELICA_PR_ERR(" %s END read size = %d ", __func__, len);
				...
				unsigned long arg)
{
	...
	FELICA_PR_ERR(" %s  START cmd = %x", __func__, cmd);
	switch (cmd) {
	...
	FELICA_PR_ERR(" %s  END cmd = %x", __func__, cmd);
	return nRet;
}
...
static unsigned int rfs_poll_poll(struct file *file, poll_table *wait)
{
	...
	FELICA_PR_ERR(" %s END mask = %d", __func__, mask);
				...
				size_t len, loff_t *ppos)
{
	...
	FELICA_PR_ERR(" %s END on = %d", __func__, on);
...
static void uartcc_init(void)
{
	...
	FELICA_PR_ERR(" %s END, major=[%d], minor=[%d]\n", __func__, \
...
static long uartcc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case UARTCC_SET_NFC_START_REQ:
		...
		FELICA_PR_ERR(" %s Set nfc start request=[%d]\n",
			__func__, guartcc_start_req);
		...
		FELICA_PR_ERR(" %s [fpga]felica_status=[%d]\n",
...
static void uartcc_felica_start_request(void)
{
	...
	FELICA_PR_ERR(" %s START\n", __func__);
	FELICA_PR_ERR(" %s start_req=[%d], felica_status=[%d]\n", \
...
{
	FELICA_PR_ERR(" %s START, status=[%d]\n", __func__, status);

	guartcc_felica_status = status;

	FELICA_PR_ERR(" %s guartcc_felica_status=[%d]\n",
			__func__, guartcc_felica_status);
	FELICA_PR_ERR(" %s END\n", __func__);
}
...
{
	FELICA_PR_ERR(" %s START, guartcc_felica_status=[%d]\n", \
			__func__, guartcc_felica_status);
	FELICA_PR_ERR(" %s END\n", __func__);
#if defined(CONFIG_MACH_KLTE_KDI)
	if(gfelica_uim_mon == 1) {
...
static unsigned int uartcc_is_idle_status(void)
{
	...
	if (!flc_cts) {	/* idle */
		FELICA_PR_ERR(" %s felica status is idle\n", __func__);
		ret = 1;
	} else {			/* else state */
		FELICA_PR_ERR(" %s felica status isn't idle=[%d]\n", \
				__func__, flc_cts);
	}
...
static long uicc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	FELICA_PR_ERR(" %s START, cmd=[%d]\n", __func__, cmd);
	FELICA_PR_ERR(" %s START, system_rev=[%d] UICC_rev[%d]\n",
	...
	case UICC_POWER_ON:
		if(system_rev < g_uicc_initrev)
		{
			...
			FELICA_PR_ERR(" %s regulator_get, reg_l9 OK]", __func__);
...
#if defined(CONFIG_MACH_KLTE_KDI)
		FELICA_PR_ERR(" %s UICC_POWER_UIM_MON! make PON always high!\n", __func__);
		gfelica_uim_mon = 1;
		gpio_set_value(gfelica_pon_pin, GPIO_VALUE_HIGH);
#else
...
static int snfc_uid_check(void)
{
	...
	FELICA_PR_ERR(" %s START", __func__);
	FELICA_PR_ERR(" %s gnfc_uid=[%d]", __func__, (int)gnfc_uid);
